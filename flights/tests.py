...
class BookingListTest(APITestCase):
    def setUp(self):
        ...
        Booking.objects.create(
            flight=flight1, date="2020-01-01", user=user1, passengers=2
        )
        Booking.objects.create(
            flight=flight2, date="2019-01-01", user=user1, passengers=2
        )
        Booking.objects.create(
            flight=flight1, date="2031-01-01", user=user2, passengers=2
        )
        Booking.objects.create(
            flight=flight2, date="2031-01-01", user=user2, passengers=2
        )

        response = self.client.post(
            reverse("login"), {"username": "laila", "password": "1234567890-="}
        )
...
class BookingDetails(APITestCase):
    def setUp(self):
        ...
        Booking.objects.create(
            flight=flight1, date="2018-01-01", user=user1, passengers=2
        )
        Booking.objects.create(
            flight=flight2, date="2019-01-01", user=user1, passengers=2
        )
        Booking.objects.create(
            flight=flight1, date="2020-01-01", user=user2, passengers=2
        )
        Booking.objects.create(
            flight=flight2, date="2021-01-01", user=user2, passengers=2
        )
    ...
    def test_url_forbidden(self):
        response = self.client.post(
            reverse("login"), {"username": "laila2", "password": "1234567890-="}
        )
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + response.data["access"])
        response = self.client.get(reverse("booking-details", args=[1]))
...
class BookingUpdate(APITestCase):
    def setUp(self):
        ...
        Booking.objects.create(
            flight=flight1, date=date.today(), user=user, passengers=2
        )
        Booking.objects.create(
            flight=flight2,
    ...
    def test_update_admin(self):
        ...
        response = self.client.post(
            reverse("login"), {"username": "admin", "password": "1234567890-="}
        )
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + response.data["access"])
...
class Login(APITestCase):
    ...
    def test_unsucceful_login(self):
        response = self.client.post(
            reverse("login"), {"username": "laila", "password": "1234567890-=1"}
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
